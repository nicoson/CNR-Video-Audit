$(function(){
	//生成组织信息树
    searchTree();
    $("#selectedDepAndMember").append(parent.$("#selectedObj").val());
});
var leftStr="";
...
function treeClick(){
    ...
    $('.dep-name').click(function(){
    	...
    	//请求部门人员信息
    	$.post($.basepath + "/setUp/memberList.do", $.param({"departmentId":departmentId}),function(result) {
    		if (result.success == true) {
    	        //显示部门员工
    ...
    $('#btnConfirm').click(function(){
    	...
    	$("#selectedDepAndMember").find(".result-text").each(function(index,element){
			...
			"</div>";
			parent.$("#membersSelection").html(selectedObj);
		});
		parent.$("#selectedObj").val(selectedObj);
		parent.$("#selectedMemberId").val(rtnMemStr.substring(1, rtnMemStr.length));
...
function showSelectedDep(departmentId, departmentName, staffList) {
    ...
    selecedDep = "<li class='c_list_item' style='padding-right:30px;position:relative;' title='" + departmentName + "' >" + 
        			 ...
        			 "<input type='checkbox' disabled='disabled' class='iconfont close' style='position:absolute;right:10px;' onclick='getSelectedObj(this);' />" + 
        		 "</li>";
